...
class ChannelAttentionModule(nn.Module):
    def __init__(self, in_chans, reduction=4):
        super(ChannelAttentionModule, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        self.fc = nn.Sequential(
            nn.Conv2d(in_chans, in_chans // reduction, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_chans // reduction, in_chans, 1, bias=False)
        )
...
class MultiScaleAttentionFusion(nn.Module):
    def __init__(self, in_chans, factor=4.0):
        super(MultiScaleAttentionFusion, self).__init__()
        dim = int(in_chans // factor)
        self.down = nn.Conv2d(in_chans, dim, kernel_size=1, stride=1)
        self.conv_3x3 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)
        self.conv_5x5 = nn.Conv2d(dim, dim, kernel_size=5, stride=1, padding=2)
        self.conv_7x7 = nn.Conv2d(dim, dim, kernel_size=7, stride=1, padding=3)
        self.spatial_attention = SpatialAttentionModule()
        self.channel_attention = ChannelAttentionModule(in_chans)
        self.up = nn.Conv2d(dim, in_chans, kernel_size=1, stride=1)
        self.down_2 = nn.Conv2d(in_chans, in_chans, kernel_size=1, stride=1)

    def forward(self, x):
        x_ori=x
        x = self.down(x)
        x_c = x_ori * self.channel_attention(x_ori)
        x_3x3 = self.conv_3x3(x)
        x_5x5 = self.conv_5x5(x)
        x_7x7 = self.conv_7x7(x)
        x_fused_s = x_3x3 + x_5x5 + x_7x7
        x_s = x_fused_s * self.spatial_attention(x_fused_s)
        x_s=self.up(x_s)

        x_out = self.down_2(x_s + x_c)
...
'''
    def __init__(self, 
        ...
        self.att = MultiScaleAttentionFusion(in_chans=in_chans)
        self.patch_embed = nn.ModuleList()
        patch_embed = PatchEmbed(img_size=img_size, patch_size=1, in_chans=in_chans, embed_dim=embed_dim[0])
